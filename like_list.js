chrome.storage.sync.get(["likedWordList"], ({ likedWordList }) => {
  ...
  for (let word of likedWordList) {
    ...
    img.addEventListener("click", () => {
      likedWordList.splice(likedWordList.indexOf(word), 1);
      ul.removeChild(li);
      ul.removeChild(hr);
      chrome.storage.sync.set({ likedWordList });
    });
  }
});
